...
// forge script script/partners/EarlyDoors.s.sol:Run --fork-url $GOERLI_RPC_URL --keystores $ETH_KEYSTORE --password $KEYSTORE_PASSWORD --sender $ETH_FROM --broadcast
contract Plurigrid is Script {
    ...
    function run() public {
        vm.startBroadcast();
        module = address(new FixedStablecoinPurchaseModule(owner, 0.001 ether, "USD", 2));
