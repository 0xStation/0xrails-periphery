...
contract FeeManagerTest is Test, SetUpMembership {
    ...
    function test_constructor(TestParams memory testParams) public {
        ...
        FeeManager.Fees memory exampleFees = FeeManager.Fees(FeeManager.FeeSetting.Set, testParams.baseFee, testParams.variableFee); 
        ...
        FeeManager.Fees memory ethFees = FeeManager.Fees(FeeManager.FeeSetting.Set, testParams.ethBaseFee, testParams.ethVariableFee); 
    ...
    function test_setDefaultFees(TestParams memory testParams, address tokenAddress) public {
        ...
        FeeManager.Fees memory mintFees = FeeManager.Fees(FeeManager.FeeSetting.Set, testParams.baseFee, testParams.variableFee); 
    ...
    function test_setDefaultFeesRevertOnlyOwner(TestParams memory testParams, address tokenAddress) public {
        ...
        FeeManager.Fees memory defaultFees = FeeManager.Fees(FeeManager.FeeSetting.Set, testParams.baseFee, testParams.variableFee); 
    ...
    function test_setOverrideFees(TestParams memory testParams, TestParams memory overrideParams, address tokenAddress) public {
        deployWithFuzz(testParams.baseFee, testParams.variableFee, testParams.ethBaseFee, testParams.ethVariableFee);
        ...
        FeeManager.Fees memory overrideFees = FeeManager.Fees(FeeManager.FeeSetting.Set, overrideParams.baseFee, overrideParams.variableFee); 
        vm.startPrank(feeManager.owner());
    ...
    function test_setOverrideFeesRevertOnlyOwner(TestParams memory testParams, TestParams memory overrideParams, address tokenAddress) public {
        deployWithFuzz(testParams.baseFee, testParams.variableFee, testParams.ethBaseFee, testParams.ethVariableFee);
    ...
    function test_getFeeTotalsBaseline(
        ...
        vm.assume(tokenAddress != address(0x0));
        _constrainFuzzInputs(testParams.baseFee, testParams.variableFee, testParams.ethBaseFee, testParams.ethVariableFee);
        _constrainFuzzInputs(overrideParams.baseFee, overrideParams.variableFee, overrideParams.ethBaseFee, overrideParams.ethVariableFee);
        ...
        uint256 erc20FeeTotal = feeManager.getFeeTotals(address(proxy), tokenAddress, someRecipient, quantity, unitPrice);
        // cast to prevent overflow
        uint256 expectedErc20FeeTotal = uint256(quantity) * uint256(testParams.baseFee) + (uint256(unitPrice) * uint256(quantity) * uint256(testParams.variableFee) / 10_000);
    ...
    function test_getFeeTotalsDefault(
        ...
        // cast to prevent overflow
        uint256 expectedEthFeeTotal = uint256(quantity) * uint256(testParams.ethBaseFee) + (uint256(unitPrice) * uint256(quantity) * uint256(testParams.ethVariableFee) / 10_000);
        assertEq(ethFeeTotal, expectedEthFeeTotal);

        // set erc20 defaultFees
        FeeManager.Fees memory defaultFees = FeeManager.Fees(FeeManager.FeeSetting.Set, defaultParams.baseFee, defaultParams.variableFee); 
        vm.prank(feeManager.owner());
        feeManager.setDefaultFees(tokenAddress, defaultFees);

        // calculate feeTotal using erc20 defaultFees
        uint256 erc20DefaultFeeTotal = feeManager.getFeeTotals(address(proxy), tokenAddress, someRecipient, quantity, unitPrice);
        // cast to prevent overflow
        uint256 expectedErc20DefaultFeeTotal = uint256(quantity) * uint256(defaultParams.baseFee) + (uint256(unitPrice) * uint256(quantity) * uint256(defaultParams.variableFee) / 10_000);
    ...
    function test_getFeeTotalsOverride(
        ...
        FeeManager.Fees memory erc20OverrideFees = FeeManager.Fees(FeeManager.FeeSetting.Set, overrideParams.baseFee, overrideParams.variableFee); 
        vm.prank(feeManager.owner());
        feeManager.setDefaultFees(tokenAddress, erc20OverrideFees);

        // calculate feeTotal using erc20 overrideFees
        uint256 erc20OverrideFeeTotal = feeManager.getFeeTotals(address(proxy), tokenAddress, someRecipient, quantity, unitPrice);
        // cast to prevent overflow
        uint256 expectedErc20OverrideFeeTotal = uint256(quantity) * uint256(overrideParams.baseFee) + (uint256(unitPrice) * uint256(quantity) * uint256(overrideParams.variableFee) / 10_000);
        assertEq(erc20OverrideFeeTotal, expectedErc20OverrideFeeTotal);

        // set eth overrideFees
        FeeManager.Fees memory ethOverrideFees = FeeManager.Fees(FeeManager.FeeSetting.Set, overrideParams.ethBaseFee, overrideParams.ethVariableFee); 
        vm.prank(feeManager.owner());
        feeManager.setDefaultFees(tokenAddress, ethOverrideFees);

        // calculate feeTotal using eth overrideFees
        uint256 ethOverrideFeeTotal = feeManager.getFeeTotals(address(proxy), tokenAddress, someRecipient, quantity, unitPrice);
        // cast to prevent overflow
        uint256 expectedEthOverrideFeeTotal = uint256(quantity) * uint256(overrideParams.ethBaseFee) + (uint256(unitPrice) * uint256(quantity) * uint256(overrideParams.ethVariableFee) / 10_000);
